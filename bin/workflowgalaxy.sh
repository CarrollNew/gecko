...
MGDIR=WWJJIITEMPFILE/${MYRAND}

mkdir $MGDIR

cp $1 $MGDIR/seqx.fasta
cp $2 $MGDIR/seqy.fasta

#$1=$MGDIR/seqx.fasta
#$2=$MGDIR/seqy.fasta

set -- "${MGDIR}/seqx.fasta" "${MGDIR}/seqy.fasta" "${@:3:7}"



BINDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
echo "$BINDIR"
dirNameX=$(${BINDIR}/readlink.sh $MGDIR/seqx.fasta | xargs dirname)
seqXName=$(basename "$MGDIR/seqx.fasta")
extensionX="${seqXName##*.}"
seqXName="${seqXName%.*}"

dirNameY=$(${BINDIR}/readlink.sh $MGDIR/seqy.fasta | xargs dirname)
seqYName=$(basename "$MGDIR/seqy.fasta")
extensionY="${seqYName##*.}"
seqYName="${seqYName%.*}"

seqXName=`basename $MGDIR/seqx.fasta .fasta`
seqYName=`basename $MGDIR/seqy.fasta .fasta`
...
mkdir $MGDIR/intermediateFiles/${seqXName}-${seqYName}
mkdir $MGDIR/results
mkdir $MGDIR/intermediateFiles/dictionaries
mkdir $MGDIR/intermediateFiles/hits

#cp $1 $MGDIR/intermediateFiles/${seqXName}-${seqYName}/${seqXName}.fasta
#cp $2 $MGDIR/intermediateFiles/${seqXName}-${seqYName}/${seqYName}.fasta

# Copiamos los fastas
ln -s ${dirNameX}/${seqXName}.${extensionX} $MGDIR/intermediateFiles/${seqXName}-${seqYName}
ln -s ${dirNameY}/${seqYName}.${extensionY} $MGDIR/intermediateFiles/${seqXName}-${seqYName}



cd $MGDIR/intermediateFiles/${seqXName}-${seqYName}
...
echo " mv ${seqXName}.d2hP ../dictionaries/ "
echo " mv ${seqXName}.d2hW ../dictionaries/"
echo " mv ${seqYName}.d2hP ../dictionaries/"
echo " mv ${seqYName}.d2hW ../dictionaries/"
echo " mv ${seqYName}-revercomp.d2hP ../dictionaries/"
echo " mv ${seqYName}-revercomp.d2hW ../dictionaries/"
...
echo "Final move: mv ${seqXName}-${seqYName}.frags $7"
mv ${seqXName}-${seqYName}.frags $7
